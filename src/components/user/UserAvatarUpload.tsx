...
export function UserAvatarUpload({
  ...
  const handleUploadOrUpdate = async () => {
    ...
    try {
      if (currentAvatar) {
        await usersApi.updateOwnAvatar(selectedFile, userId);
        toast.success("تم تحديث الصورة الشخصية بنجاح");
      } else {
        await usersApi.uploadOwnAvatar(selectedFile, userId);
  ...
  const handleRemoveAvatar = async () => {
    setIsLoading(true);
    try {
      await usersApi.removeOwnAvatar(userId);
  ...
  return (
    <>
      <div className={`relative ${className}`}>
        <div className="relative w-20 h-20 rounded-full bg-gradient-to-br from-purple-500 to-pink-600 p-0.5 shadow-lg">
          ...
          <Button
            variant="secondary"
            size="sm"
            className="absolute -bottom-1 -right-1 rounded-full w-8 h-8 p-0 bg-white dark:bg-gray-800 border-2 border-purple-500 hover:bg-gray-50 dark:hover:bg-gray-700 shadow-lg"
      ...
      <Dialog open={showDialog} onOpenChange={handleCloseDialog}>
        <DialogContent className="sm:max-w-[500px]" dir="rtl">
          <DialogHeader>
            ...
            <DialogDescription>إدارة صورتك الشخصية</DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-6 py-4">
            ...
            <div>
              <Label className="text-sm font-medium block mb-3">
                {currentAvatar ? "تحديث الصورة" : "إضافة صورة جديدة"}
              </Label>
